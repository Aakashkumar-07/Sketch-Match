...
export function DrawingCanvas() {
    ...
    if (editingText.text.trim()) {
      ...
      if (existingShape) {
        ...
        const textShape: CanvasShape = {
          ...
          width: editingText.text.length * 8, 
  ...
  const draw = useCallback(() => {
    ...
    if (collaborationMode && remoteCursors.length > 0) {
      remoteCursors.forEach((cursor) => {
        drawRemoteCursor(ctx, cursor.x, cursor.y, cursor.userName, cursor.userColor)
      })
    }
  ...
  const drawShape = (ctx: CanvasRenderingContext2D, shape: CanvasShape) => {
    ...
    if (isSelected) {
      ...
      const handles = [
        ...
        { x: shape.x + (shape.width || 0) - handleSize / 2, y: shape.y - handleSize / 2 }, 
        { x: shape.x - handleSize / 2, y: shape.y + (shape.height || 0) - handleSize / 2 }, 
        { x: shape.x + (shape.width || 0) - handleSize / 2, y: shape.y + (shape.height || 0) - handleSize / 2 }, 
        { x: shape.x + (shape.width || 0) / 2 - handleSize / 2, y: shape.y - handleSize / 2 }, 
        { x: shape.x + (shape.width || 0) / 2 - handleSize / 2, y: shape.y + (shape.height || 0) - handleSize / 2 }, 
        { x: shape.x - handleSize / 2, y: shape.y + (shape.height || 0) / 2 - handleSize / 2 }, 
        { x: shape.x + (shape.width || 0) - handleSize / 2, y: shape.y + (shape.height || 0) / 2 - handleSize / 2 }, 
      ]
  ...
  const drawRemoteCursor = (ctx: CanvasRenderingContext2D, x: number, y: number, name: string, color: string) => {
    const cursorSize = 16 / stageScale
    ...
    ctx.lineTo(x + cursorSize * 0.4, y + cursorSize * 0.7)
    ctx.lineTo(x + cursorSize * 0.7, y + cursorSize * 0.7)
    ctx.closePath()
    ctx.fill()
    ctx.stroke()
    ...
    const labelX = x + cursorSize * 0.7
    const labelY = y + cursorSize * 0.7

    
    ctx.fillStyle = color
    const radius = 3 / stageScale
    const rectX = labelX
    const rectY = labelY
    ...
    ctx.fillText(name, labelX + padding, labelY + fontSize + padding * 0.5)
  }
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (collaborationMode) {
      const cursorPos = getCanvasCoordinates(e.clientX, e.clientY)
      collaborationManager.broadcastCursor(cursorPos.x, cursorPos.y)
    }
